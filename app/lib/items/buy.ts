import { getLanguageCode } from "lib/i18n/iso6391";
import { Item } from "types/Item";

export const buy: Item[] = [
  {
    category: "buy",
    link: "https://lgt.st/swan",
    image: "swan.jpeg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAP0lEQVR4nAE0AMv/AM3i9wlHbAA5X8/j+AAAK1LB1+1uja0ABDUABEFn7///vNToAClRANnt/z5oigA0XN3w/32KGWQSLeGBAAAAAElFTkSuQmCC",
    name: "Swan",
    slogan:
      "Swan Bitcoin provides savings plans, instant buys, Private Client for businesses and HNWI, Advisor Services for financial advisors, and world-class Bitcoin education.",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/strike",
    image: "strike.webp",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAPoAAAD6AG1e1JrAAAAJUlEQVR4nGNgQAYiIiL79++/deuWoKAgiJ+RkfHr1y9PT08UVQAE+Ao8BpEpowAAAABJRU5ErkJggg==",
    name: "Strike",
    slogan: "Send and receive money instantly.",
    platforms: ["web", "mobile", "desktop"],
  },
  {
    category: "buy",
    link: "https://lgt.st/cashapp",
    image: "cashapp.png",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAMElEQVR4nGNg0GRgsGBgSGYAMRiUGNre5c//38GgzgASqHmWuvb/NLCMJliNI4gBABEUCpGscxy/AAAAAElFTkSuQmCC",
    name: "Cash App",
    slogan:
      "The #1 finance app in the App Store. Pay anyone, instantly. Design a debit card to match your style. Invest in stocks or bitcoin with as little as $1.",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/river",
    image: "river.jpeg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAN0lEQVR4nGPg4uISERERFxfj4uJiYGBg+P/n2Z93N0AsVRWl57ePnN67Wl1VhUFQUJAVDAQFBQBKDQx3tn6HcAAAAABJRU5ErkJggg==",
    name: "River",
    slogan:
      "Easily buy, sell and mine Bitcoin in the USA. River has world-class security, leading customer support, and zero fees on recurring orders.",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/azteco",
    image: "azteco.jpeg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAOElEQVR4nGP48P7t8xfPX7589e7jBwZeNfOAwDBbN1sNATkGBlG1mIQUXTM9EOfz5w+PHz+CKAMAu6ka0GnYBDMAAAAASUVORK5CYII=",
    name: "Azteco",
    slogan: "Azteco: the easiest way to buy Bitcoins.",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/noah",
    image: "noah.jpeg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAOUlEQVR4nGOIZ2CAoBwGBoYbagz/23z+tzo/t2Fg+Bcu9X9BzP+jlf+rVBmSGBiKGBhaGBjWMTAAAKEsEI7xoz7UAAAAAElFTkSuQmCC",
    name: "NOAH",
    slogan:
      "Send, Save, Pay and Earn with NOAH, the all-in-one money app of the future.",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/amber",
    image: "amber.jpeg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAP0lEQVR4nAE0AMv/AP///9rb2/Hx8f///wDd3d0aHB1dX1////8Aubm5AAUOQzwt892vAPv6+tfY2evn4vvz45T/IsURi+WbAAAAAElFTkSuQmCC",
    name: "amber",
    slogan:
      "🌏 Amber app is expanding and bringing easy Bitcoin accumulation to a country near you.",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/shakepay",
    image: "shakepay.jpeg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAOElEQVR4nGNgENJkEFIDIw0GTteE3t//O378Z7AMYmDQcl33///8//8ZlG0YGFiVGcR1GMQ0gQwAbSIODqPW9PwAAAAASUVORK5CYII=",
    name: "shakepay",
    slogan:
      "Shakepay allows Canadians to buy/sell bitcoin and pay their friends. We're on a mission to create open access to building wealth.",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/bullbitcoin",
    image: "bullbitcoin.png",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAJklEQVR4nGOIZWCAI4YcBobfbb3/OidkMYAF/j948v/eI5AMsjIAbNAOIO3hlRQAAAAASUVORK5CYII=",
    name: "Bull Bitcoin",
    slogan:
      "Bull Bitcoin is a non-custodial Bitcoin exchange and payments service. Our mission is to empower Canadians to take back control over their money.",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/pocket",
    image: "pocket.jpeg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAPElEQVR4nGPQZ9iYwLA3jGGHPsNGBjOGTe9P/P//478GwxoGC4ZNb079n5y5k4FhIojjyLCFgaFelGEdACj4FFqC4nZ2AAAAAElFTkSuQmCC",
    name: "Pocket",
    slogan:
      "Experience the best way to buy and store Bitcoin. From your bank account directly into your self-custody. One-off or recurring at the most recent exchange rate.",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/casa",
    image: "casa.jpeg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAANklEQVR4nGNgYOhkYJjCwDCHgWEGAwPDhEfn/+9c+J+BYSqI8//9/0t7oJxOBoY+BoYJDAwzAbscEV8Aou+YAAAAAElFTkSuQmCC",
    name: "Casa",
    slogan:
      "Casa is the leading provider of Bitcoin self-custody solutions. Buy, sell, and receive bitcoin with our ultimate blend of security, privacy, and control.",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/peach",
    image: "peach.jpeg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAPUlEQVR4nGOIYGCYws+wJ0hiXwQDQyIDw/+tc/+/OrHBiZkhn4Hh/+MLj5sy2xgYGCIYGEoZGMIYGGIYGADVsBCEgNfnOgAAAABJRU5ErkJggg==",
    name: "Peach",
    slogan: "Peach: Buy and Sell bitcoin the way you want!",
    platforms: ["web", "mobile"],
  },
  {
    category: "buy",
    link: "https://lgt.st/coinbits",
    image: "coinbits.jpg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAPUlEQVR4nGOIYGCYws+wJ0hiXwQDQyIDw/+tc/+/OrHBiZkhn4Hh/+MLj5sy2xgYGCIYGEoZGMIYGGIYGADVsBCEgNfnOgAAAABJRU5ErkJggg==",
    name: "Coinbits",
    slogan:
      "Coinbits is the best choice for people who want to get started with bitcoin quickly and safely, with no long-term commitment.",
    platforms: ["web"],
  },
  {
    category: "buy",
    link: "https://lgt.st/relai",
    image: "relai.png",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAPUlEQVR4nGOIYGCYws+wJ0hiXwQDQyIDw/+tc/+/OrHBiZkhn4Hh/+MLj5sy2xgYGCIYGEoZGMIYGGIYGADVsBCEgNfnOgAAAABJRU5ErkJggg==",
    name: "Relai",
    slogan:
      "Relai is a Bitcoin investment app that allows anyone to buy bitcoin within one minutes or set up an automated savings plan.",
    platforms: ["web"],
  },
  {
    category: "buy",
    link: "https://lgt.st/bottlepay",
    image: "bottlepay.jpeg",
    languageCodes: [getLanguageCode("English")],
    placeholderDataUrl:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAIAAAAmkwkpAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAPUlEQVR4nGOIYGCYws+wJ0hiXwQDQyIDw/+tc/+/OrHBiZkhn4Hh/+MLj5sy2xgYGCIYGEoZGMIYGGIYGADVsBCEgNfnOgAAAABJRU5ErkJggg==",
    name: "Bottlpay",
    slogan:
      "Bottlepay makes buying bitcoin easy. Make a one-off purchase or set up a recurring trade hourly, daily, weekly or monthly.",
    platforms: ["web"],
  },
];
